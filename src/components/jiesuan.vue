<template>
  <div class="jiesuan">
    ...
    <!-- 购物车信息 -->
    <div class="gwcxx">
      <div class="cart" v-for="(item,i) in $store.state.cart.goods" :key="i">
        <div class="tp">
        ...
        <div class="xx">
          ...
          <div class="jg">&yen; {{formatPrice(item.price)}}</div>
        </div>
      </div>
    </div>
    <!-- 购物车信息end -->
  ...
  methods: {
    ...
    qrzf() {
      ...
      this.$store.state.cart.goods.forEach(item => {
        item.time = new Date;
        item.ddh = '191128'+String(new Date().getTime()).slice(-4);
        item.isddh = true;
        this.$store.state.jiesuan.order.push(item);
      });
      //获取激活状态,判断是那个支付方式
